...
if(!nu.fixed){
  ...
  while((criterio>=prec)&&(cont<=max.iter)&&(bb>=0)){
    ...
    V<-optim(nu,logsslnu,gr=NULL,y,X,beta0,Sigma,lambda,method="L-BFGS-B",lower=nu.min,upper=40,control=list(fnscale=-1,maxit=50))   
    nu<-as.numeric(V$par) 
...
if(conv.problem==0) object.out<-list(coefficients=P[,1],se=P[,2],nu=nu,logLik=logL,AIC=AIC,BIC=BIC,iterations=cont,time=tempo,conv=conv,dist="MSSL2",class="MSMSN",n=nrow(y))
else{
object.out<-list(coefficients=P[,1],nu=nu,logLik=logL,AIC=AIC,BIC=BIC,iterations=cont,time=tempo,conv=conv,dist="MSSL2",class="MSMSN",n=nrow(y))
object.out$warnings="Standard errors can't be estimated: Numerical problems with the inversion of the information matrix"
 ...
 object.out$y<-y.or
 object.out$X<-X.or
 object.out$"function"<-"estimate.MSSL2"
 object.out
 }

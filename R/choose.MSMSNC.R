...
grepl("Error",fit.mscec)[1] & grepl("Error",fit.msnc)[1])
  ...
  fitted<-c("MSTEC","MSSLEC","MSCEC","MSNC")[index]
  index<-index[which.min(maxi)]
  fit<-switch(index, "1"=fit.mstec, "2"=fit.msslec, "3"=fit.mscec, "4"=fit.msnc)
  selected<-c("MSTEC","MSSLEC","MSCEC","MSNC")[index]
  se<-"Error"
  if(est.var)
  {
   if(selected=="MSCEC") se<-try(se.est(P=fit$coefficients,y=y,X=X,dist=selected,nu=fit$nu,gamma=fit$gamma),silent=TRUE)

...
if(dist=="MSCN2" | dist=="MSCEC") {if(!exists("nu.fixed")) nu.fixed=0.1; if(!exists("gamma.fixed")) gamma.fixed=0.5;
...
if(is.numeric(gamma.fixed) & (as.numeric(gamma.fixed)<0 | as.numeric(gamma.fixed)>1)) stop("gamma fixed must be a number between 0 and 1")}
    ...
    X.complete <- X
    m = ncol(X.complete[[1]])
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") fit.complete<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=FALSE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") fit.complete<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, nu.min=nu.min)
  if(dist=="MSCEC" | dist=="MSCN2") fit.complete<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
...
if(length(ind.interc)!=q)
    ...
    lista.final <- list(coefficients = fit.complete$coefficients, logLik = fit.complete$logLik, 
        AIC = fit.complete$AIC, BIC = fit.complete$BIC, conv = fit.complete$conv,
        dist = fit.complete$dist, class = fit.complete$class,
         comment = "The final model considered all the betas")
   if (dist == "MSTEC" | dist == "MSSLEC" | dist== "MSTT" | dist=="MSSL2") lista.final$nu = fit.complete$nu
   if (dist == "MSCEC" | dist == "MSCN2") lista.final$nu = fit.complete$nu; lista.final$gamma = fit.complete$gamma
    historico <- c()
    flag <- FALSE
    while (!flag) {
  ...
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") fit.complete.j<-estimate.dist(y=y,X=X.complete.j,max.iter=max.iter,prec=prec, est.var=FALSE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") fit.complete.j<-estimate.dist(y=y,X=X.complete.j,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, nu.min=nu.min)
  if(dist=="MSCEC" | dist=="MSCN2") fit.complete.j<-estimate.dist(y=y,X=X.complete.j,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
                if (criteria == "AIC") 
  ...
  if(dist=="MSCEC" | dist=="MSCN2") fit.complete.j<-estimate.dist(y=y,X=X.complete.j,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
        ...
        crit.complete <- ifelse(criteria == "AIC", fit.complete$AIC, 
            fit.complete$BIC)
        ...
        if (min(criteria.complete) < crit.complete) {
            X.complete <- lapply(X.complete, function(x) {
                x[, -ind, drop=FALSE]})
            historico <- c(historico, rownames(fit.complete$coefficients)[1:m][ind])
            names.betas <- rownames(fit.complete$coefficients)[1:m][-ind]
            tt <- table(unlist(sapply(X, pos.ones)))
            ind.interc <- as.numeric(names(tt)[which(tt == nrow(y))])
            m = ncol(X.complete[[1]])
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") fit.complete.j<-estimate.dist(y=y,X=X.complete,max.iter=max.iter,prec=prec, est.var=FALSE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") fit.complete.j<-estimate.dist(y=y,X=X.complete,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, nu.min=nu.min)
  if(dist=="MSCEC" | dist=="MSCN2") fit.complete.j<-estimate.dist(y=y,X=X.complete,max.iter=max.iter,prec=prec, est.var=FALSE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
            rownames(fit.complete$coefficients)[1:m] <- names.betas
...
lista.final$y=fit.complete$y
            rownames(lista.final$coefficients)[1:m] <- names.betas
        }
    }
...
{names.betas=paste("beta",(1:(ncol(X[[1]])))[-as.numeric(substring(fit$eliminated,5,10))],sep="");
 names(fit$coefficients)[1:length(names.betas)]=names.betas}
  ...
  if(fit$dist=="MSTEC" | fit$dist=="MSSLEC" | fit$dist=="MSTT" | fit$dist=="MSSL2") se<-try(se.est(fit$coefficients,y,lista.final$X,dist=fit$dist,nu=fit$nu),silent=TRUE)
  if(fit$dist=="MSCEC" | fit$dist=="MSCN2") se<-try(se.est(fit$coefficients,y,lista.final$X,dist=fit$dist,nu=fit$nu,gamma=fit$gamma),silent=TRUE)
  } 
  if(!grepl("Error",se[1])){

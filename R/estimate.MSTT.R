...
X<-Xs}
  ...
  while((criterio>=prec)&&(cont<=max.iter)&&(bb>=0)){
    ...
    nu=optimize(logsttnu,c(2.0001,100),y,X,beta0,Sigma,lambda,maximum=T) #nu,Y,X,beta0,Sigma,lambda
 ...
 colnames(P)<-c("estimate","s.e.")
  ...
  ll<-list(estimate=P,nu=nu,logLik=logL,AIC=AIC,BIC=BIC,iterations=cont,time=tempo,conv=conv)

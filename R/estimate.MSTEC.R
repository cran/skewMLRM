...
if(nu.fixed!=FALSE & !is.numeric(nu.fixed)) 
   stop("nu fixed must be a number greater than 2")
if(is.numeric(nu.fixed) & as.numeric(nu.fixed)<2)
   stop("nu fixed must be a number greater than 2")
if(!is.matrix(y))
...
lsmsnc.prof<-function(nu,y,X,beta,Sigma,eta,dist=c("STEC"))
{
aux.profile.llike.MSMSNC<-function(x,p,d,A,dist=c("STEC"),nu=1){
logf<-dgamma(x,shape=(nu+p)/2,rate=(nu+p)/2,log=TRUE)
log.int<-pcauchy(sqrt((nu+p)*x/(nu+d))*A,log.p=TRUE)+logf
exp(log.int)} 
 n=nrow(y)
  p=ncol(y)  
  m=ncol(X[[1]])
...
min.int<-qgamma(0.001, shape=(nu+p)/2,scale=(nu+p)/2)
max.int<-qgamma(0.999, shape=(nu+p)/2,scale=(nu+p)/2)
aux.int<-integrate(aux.profile.llike.MSMSNC,lower=min.int,upper=max.int,p=p,d=d,A=A,dist=dist,nu=nu,abs.tol=1e-15)$value
ll[i]<-log(2)+mvtnorm::dmvt(c(y[i,]-mu), sigma=Sigma, df=nu, log=TRUE)+log(aux.int)
}
-sum(ll)
}
E.step.MSMSNC.par<-function(y,X,beta,Sigma,eta,dist=c("STEC"),nu=1)
{
compute.a1<-function(a,d,p,dist=c("STEC"),nu=1)
{
aux.a1.MSMSNC<-function(x,a,d,p,dist=c("STEC"),nu=1){
logf<-dgamma(x,shape=(nu+p)/2,rate=(nu+d)/2)
log.int<-x*pcauchy(sqrt(x)*a)*logf
return(log.int)}
max.int<-Inf
integrate(aux.a1.MSMSNC,lower=0,upper=max.int,a,d,p,dist=dist,nu=nu,abs.tol=1e-12)$value
}
compute.b1<-function(a,d,p,dist=c("STEC"),nu=1)
{
aux.b1.MSMSNC<-function(x,a,d,p,dist=c("STEC"),nu=1){
logf<-dgamma(x,shape=(nu+p)/2,rate=(nu+d)/2)
...
compute.c1<-function(a,d,p,dist=c("STEC"),nu=1)
{
aux.c1.MSMSNC<-function(x,a,d,p,dist=c("STEC"),nu=1){
logf<-dgamma(x,shape=(nu+p)/2,rate=(nu+d)/2)
...
compute.den.MSMSNC<-function(a,d,p,dist=c("STEC"),nu=1)
{
 aux.den.MSMSNC<-function(x,a,d,p,dist=c("STEC"),nu=1){  
logf<-dgamma(x,shape=(nu+p)/2,rate=(nu+d)/2)
...
nu.new<-optim(nu.last,lsmsnc.prof,method="Brent",lower=lower1,upper=30,y=y,X=X,beta=beta.new,Sigma=Sigma.new,eta=eta.new,dist=dist,control = list(maxit = 1000))$par
...
eta.last<-matrix(eta0,ncol=1)
nu.last<-nu.fixed
  lower1<-switch(dist, STEC=2.0001, SSLEC=1.0001)
i=0;dif=10
while(i<=max.iter && dif>prec)
{  aux<-E.step.MSMSNC.par(y,X,c(beta.last),Sigma.last,c(eta.last),dist,nu=nu.last)
   a.theta<-aux$a.theta
...
rownames(P)<-c(paste("beta",1:m,sep=""),paste("alpha",indices,sep=""),paste("lambda",1:p,sep=""))
 if(conv.problem==0)  ll<-list(coefficients=P[,1],se=P[,2],nu=nu,logLik=lognu,AIC=AIC,BIC=BIC,iterations=i,time=tempo,conv=conv,dist="MSTEC",class="MSMSNC",n=nrow(y))
 else{
 ll<-list(coefficients=P[,1],nu=nu,logLik=lognu,AIC=AIC,BIC=BIC,iterations=i,time=tempo,conv=conv,dist="MSTEC",class="MSMSNC",n=nrow(y)) 
 ll$warnings="Standard errors can't be estimated: Numerical problems with the inversion of the information matrix"
 }
 class(ll) <- "skewMLRM"
 ll$y<-y.or
 ll$X<-X.or
 ll$"function"<-"estimate.MSTEC"
 ll
}
  n=nrow(y)
  p=ncol(y)
  ...
  nu0<-3
EM.SMSNC(y,X,beta0,Sigma0,eta0,nu0,dist="STEC",max.iter=max.iter,prec=prec,est.var=est.var)
}

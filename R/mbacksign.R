...
mf <- match.call(expand.dots = FALSE)
if("nu.fixed" %in% names(mf$...)) nu.fixed=mf$...$nu.fixed
if("gamma.fixed" %in% names(mf$...)) gamma.fixed=mf$...$gamma.fixed
if("nu.min" %in% names(mf$...)) nu.min=mf$...$nu.min
if(dist=="MSTT" | dist=="MSSL2" | dist=="MSTEC" | dist=="MSSLEC") {if(!exists("nu.fixed")) nu.fixed=3; if(!exists("nu.min")) nu.min=2.0001; if(!exists("nu.fixed")) nu.fixed=TRUE; 
     if(!is.numeric(nu.min) | nu.min<=0) stop("nu.min should be a positive number");
if(nu.fixed!=FALSE & !is.numeric(nu.fixed)) stop("nu fixed must be a number greater than 1")
if(is.numeric(nu.fixed) & as.numeric(nu.fixed)<1) stop("nu fixed must be a number greater than 1")}
if(dist=="MSCN2" | dist=="MSCEC") 
     {if(!exists("nu.fixed")) nu.fixed=0.1; if(!exists("gamma.fixed")) gamma.fixed=0.5;
if(nu.fixed!=FALSE & !is.numeric(nu.fixed)) stop("nu fixed must be a number between 0 and 1")
if(is.numeric(nu.fixed) & (as.numeric(nu.fixed)<0 | as.numeric(nu.fixed)>1)) stop("nu fixed must be a number between 0 and 1")
if(gamma.fixed!=FALSE & !is.numeric(gamma.fixed)) stop("gamma fixed must be a number between 0 and 1")
if(is.numeric(gamma.fixed) & (as.numeric(gamma.fixed)<0 | as.numeric(gamma.fixed)>1)) stop("gamma fixed must be a number between 0 and 1")}
...
X<-Xs}
  ...
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") object.1<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=TRUE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") object.1<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=TRUE, nu.fixed=nu.fixed, nu.min=nu.min)
  if(dist=="MSCEC" | dist=="MSCN2") object.1<-estimate.dist(y=y,X=X,max.iter=max.iter,prec=prec, est.var=TRUE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
  beta.est <- object.1$"coefficients"[c(1:m)[-ind.interc]]
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") se<-try(se.est(object.1$"coefficients",y,X,dist=dist),silent=TRUE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") se<-try(se.est(object.1$"coefficients",y,X,dist=dist,nu=object.1$"nu"),silent=TRUE)
  if(dist=="MSCEC" | dist=="MSCN2") se<-try(se.est(object.1$"coefficients",y,X,dist=dist,nu=object.1$"nu",gamma=object.1$"gamma"),silent=TRUE)
...
logLik=object.1$"logLik", AIC=object.1$"AIC",BIC=object.1$"BIC", conv=object.1$conv,
             dist = object.1$dist, class = object.1$class,
             comment="The final model considered all the betas")
       lista.final$dist = object.1$"dist"
       lista.final$class = object.1$"class"
       lista.final$X<-X
   if (dist == "MSTEC" | dist == "MSSLEC" | dist== "MSTT" | dist=="MSSL2") lista.final$nu = object.1$nu
   if (dist == "MSCEC" | dist == "MSCN2") lista.final$nu = object.1$nu; lista.final$gamma = object.1$gamma
...
names.betas<-rownames(object.1$"coefficients")[1:m][-(t.min)]
      xmenos.multi <- lapply(xmenos.multi, function(x) { x[,-(t.min),drop=FALSE]})
      m=ncol(xmenos.multi[[1]])
      tt<-table(unlist(sapply(xmenos.multi,pos.ones)))
      ind.interc<-as.numeric(names(tt)[which(tt==nrow(y))])
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") object.1<-estimate.dist(y=y,X=xmenos.multi,max.iter=max.iter,prec=prec, est.var=TRUE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") object.1<-estimate.dist(y=y,X=xmenos.multi,max.iter=max.iter,prec=prec, est.var=TRUE, nu.fixed=nu.fixed, nu.min=nu.min)
  if(dist=="MSCEC" | dist=="MSCN2") object.1<-estimate.dist(y=y,X=xmenos.multi,max.iter=max.iter,prec=prec, est.var=TRUE, nu.fixed=nu.fixed, gamma.fixed=gamma.fixed)
rownames(object.1$"coefficients")[1:m]<-names.betas
      beta.est<- object.1$"coefficients"[c(1:m)[-ind.interc]]
  if(dist!="MSTEC" & dist!="MSSLEC" & dist!="MSCEC" & dist!="MSTT" & dist!="MSSL2" & dist!="MSCN2") se<-try(se.est(object.1$"coefficients",y,X=xmenos.multi,dist=dist),silent=TRUE)
  if(dist=="MSTEC" | dist=="MSSLEC" | dist=="MSTT" | dist=="MSSL2") se<-try(se.est(object.1$"coefficients",y,X=xmenos.multi,dist=dist,nu=object.1$"nu"),silent=TRUE)
  if(dist=="MSCEC") se<-try(se.est(object.1$"coefficients",y,X=xmenos.multi,dist=dist,nu=object.1$"nu",gamma=object.1$"gamma"),silent=TRUE)
      if(!is.numeric(se)) stop("Standard errors can't be estimated: Numerical problems with the inversion of the information matrix")
      ep.beta <- se[c(1:m)[-ind.interc]]
      dim.beta <- length(beta.est)
      test.t <- abs(beta.est/ep.beta)
...
lista.final$conv=object.1$"conv"
       lista.final$comment=paste("The final model eliminated",length(historico),"betas")
lista.final$"eliminated"=historico
       lista.final$X=xmenos.multi
    } } 
  se<-"Error";   fit<-lista.final
  if(fit$dist!="MSTEC" & fit$dist!="MSSLEC" & fit$dist!="MSCEC" & fit$dist!="MSTT" & fit$dist!="MSSL2" & dist!="MSCN2") se<-try(se.est(P=fit$coefficients,y=y,X=fit$X,dist=fit$dist),silent=TRUE)
  if(fit$dist=="MSTEC" | fit$dist=="MSSLEC" | fit$dist=="MSTT" | fit$dist=="MSSL2") se<-try(se.est(P=fit$coefficients,y=y,X=fit$X,dist=fit$dist,nu=fit$nu),silent=TRUE)
  if(fit$dist=="MSCEC" | fit$dist=="MSCN2") se<-try(se.est(P=fit$coefficients,y=y,X=fit$X,dist=fit$dist,nu=fit$nu,gamma=fit$gamma),silent=TRUE)
